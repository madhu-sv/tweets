...
public class LambdaAuthorizer implements RequestHandler<TokenAuthorizerContext, AuthPolicy> {
    ...
    public AuthPolicy handleRequest(TokenAuthorizerContext input, Context context) {
        ...
        // this function must generate a policy that is associated with the recognised principal user identifier.
        // depending on your use case, you might store policies in a DB, or generate them on the fly

        // keep in mind, the policy is cached for 5 minutes by default (TTL is configurable in the authoriser)
